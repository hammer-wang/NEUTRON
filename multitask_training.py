...
from mdn.model import MultitaskMixtureDensityNetwork

illuminant = SDS_ILLUMINANTS['D65']
cmfs = MSDS_CMFS['CIE 1931 2 Degree Standard Observer']

THICKNESS_SCALER = joblib.load(os.path.join(
    './simulation/multilayer_data/sRGB_400K', 'thickness_scaler.joblib'))
NK_SCALER = joblib.load(os.path.join(
    './simulation/multilayer_data/sRGB_400K', 'nk_scaler.joblib'))
RGB_SCALER = joblib.load(os.path.join(
    './simulation/multilayer_data/sRGB_400K', 'rgb_scaler.joblib'))
...
class MultitaskMDN(pl.LightningModule):
    ...
    def __init__(self, lr, hidden_dim=256, input_dim=310, n_mixtures=5, weight_decay=1e-5, alpha=1, train_ratio=1.0, dataset='400K', seed=42):
        ...
        self.datapath = './simulation/multilayer_data/sRGB_400K'

        self.mdn = MultitaskMixtureDensityNetwork(
            input_dim, 4, n_mixtures, hidden_dim, alpha)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            thickness_pred = self.mdn.sample(nk, designed_rgb)[
                0].detach().cpu().numpy()
            thickness_pred = np.hstack(
                (thickness_pred, np.ones((len(thickness_pred), 1))))
            thickness_pred = THICKNESS_SCALER.inverse_transform(
                thickness_pred).astype(int)
            thickness_pred[:, 4] = 100
            thickness_pred = thickness_pred.tolist()

            # get nk_dict
            nk_dicts = []
            nk = NK_SCALER.inverse_transform(
                nk.detach().cpu().numpy()).reshape(-1, 10, 31)
            ...
            designed_lab = pool.starmap(
                get_color, zip(nk_dicts, thickness_pred))

            deltaE = np.mean([delta_E_CIE2000(lab1, lab2)
                             for lab1, lab2 in zip(designed_lab_, designed_lab)])
            print(f'deltaE {deltaE}')

            pred = torch.sigmoid(out[-1])

            bce_loss = self.mdn.alpha * \
                torch.nn.BCEWithLogitsLoss()(torch.squeeze(out[-1]), labels)
    ...
    def train_dataloader(self):
        ...
        if self.train_ratio < 1:
            ...
            train_indices = rng.choice(
                np.arange(len(train_set)), num_train, replace=False)
    ...
    def compute_deltaE(self, nk, nk_dicts, rgb, lab):
        ...
        deltaE = np.mean([delta_E_CIE2000(lab1, lab2)
                         for lab1, lab2 in zip(lab, lab_pred)])
        print(
            f'Lab evaluation time {end_time - start_time:.1f} sec, deltaE {deltaE}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', choices=('400K'), default='400K')
    ...
    def get_color(nk_dict, thickness):
        ...
        for i, lambda_vac in enumerate(wavelengths * 1e3):
            ...
            res = inc_tmm(
                's', n_list, [np.inf] + thickness + [5e3, np.inf], inc_list, 0, lambda_vac)

            res_t = inc_tmm(
                'p', n_list, [np.inf] + thickness + [5e3, np.inf], inc_list, 0, lambda_vac)
    ...
    input_dim = 310
    model = MultitaskMDN(lr, args.hidden_dim, input_dim, args.n_mixtures,
                         args.weight_decay, args.alpha, args.train_ratio, args.dataset, args.seed)
